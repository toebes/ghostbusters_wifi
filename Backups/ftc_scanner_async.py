...
# Flag variable to stop scan code
run_scan_signal = False
event_scan_signal = Event()
...
def scan_single_thread(event, ssid, bssid, channel):
    while run_scan_signal:
        scan = scan_single(ssid=ssid, bssid=bssid, channel=channel)
        event.set(scan)
...
def start_up():
    ...
    value = 0.9
    picogui.draw_jpg(filename=SCANNING)
...
def TrackWifi(picogui, item):
    ...
    show_memory('in TrackWifi')

    run_scan_signal = True
    _thread.start_new_thread(scan_single_thread(
        event=event_scan_signal, ssid=item[0], bssid=item[1], channel=item[2]))
...
#        if elapsed_time > 500:
#            print(f'Time to rescan')
#            new_item = scan_single(ssid=item[0], bssid=item[1], channel=item[2])
        if event_scan_signal.is_set:
            new_item = event_scan_signal.value()
            if len(new_item) > 0:
                item = new_item
            else:
                item[4] = "0"
